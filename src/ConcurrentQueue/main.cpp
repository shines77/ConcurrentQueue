...
public:
    ...
    void onValueChange2(int index, int x, int y) {
        std::cout << "FooA::onValueChange2(int index, int x, int y): index = " << index << std::endl;
    }
};
...
public:
    ...
    void onValueChange2(int index, int x, int y) {
        std::cout << "FooB::onValueChange2(int index, int x, int y): index = " << index << std::endl;
    }
};

/*
template <typename R, typename Arg>
bool function_equal(std::function<R(Arg)> const & func1, std::function<R(Arg)> const & func2) _NOEXCEPT {
    R (* const * ptr1)(Arg) = func1.target<R(*)(Arg)>();
    R (* const * ptr2)(Arg) = func2.target<R(*)(Arg)>();
...
bool function_equal(std::function<R(Args...)> const & func1, std::function<R(Args...)> const & func2) _NOEXCEPT {
    R (* const * ptr1)(T *, Args...) = func1.template target<R(*)(T *, Args...)>();
    R (* const * ptr2)(T *, Args...) = func2.template target<R(*)(T *, Args...)>();
    return ((ptr1 != nullptr) && (ptr2 != nullptr) && (ptr1 == ptr2));
}
//*/
...
void test_signal()
{
    ...
    signal_int_inst.connect(signal_slot::OnValueChange, memfunc_a1);
